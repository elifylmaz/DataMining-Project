...
Original file is located at
    https://colab.research.google.com/drive/1lAFezyPJ82vk_12QMVIrNnaH2dLKQfWR
"""
...
!pip install nltk
import nltk
nltk.download('punkt_tab') # download the missing package

!pip install nltk
import nltk
nltk.download('punkt_tab')  # Download the 'punkt_tab' data package
nltk.download('punkt')  # 'punkt' tokenizer veri setini tekrar indir.
nltk.download('stopwords')  # Durma kelimeleri setini tekrar indir.
nltk.download('wordnet')  # Lemmatization için gerekli wordnet veri setini indir.
...
print(df[['Review', 'Lemmatized_Review']].head())  # İlk birkaç satırı göster
...
file_path = "cleaned_reviews.csv"  # Dosya yolu
...
def label_sentiment(review):
    """
    Yorumların duygusal analizini yapar ve etiketler:
...
# 5. Sonuçları Görüntüleme
print(df[['Review', 'Label']].head())  # İlk 5 satırı görüntüle

# 6. Etiketli veriyi yeni bir dosyaya kaydetme
output_path = "auto_labeled_reviews.csv"
df.to_csv(output_path, index=False)
...
# Yalnızca gerekli sütunları tutma
df_reduced = df[['Review', 'Lemmatized_Review', 'Label']]

# İlk 10 satırı yazdırma
df_reduced.head(10)
#Pozitif: 1574 , Nötr: 518 , Negatif: 441
